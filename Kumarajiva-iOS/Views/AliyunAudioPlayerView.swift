...
// MARK: - 阿里云盘音频播放器视图（重构版）
struct AliyunAudioPlayerView: View {
    ...
    private func prepareAudio() {
        print("☁️ [AliyunAudioPlayer] 准备音频播放: \(file.name)")
        
        let isSameFile = playerService.playbackState.currentEpisode?.id == file.fileId
        
        if !isSameFile {
            playerService.clearCurrentPlaybackState()
            print("☁️ [AliyunAudioPlayer] 切换到新文件，清空播放状态")
        } else {
            print("☁️ [AliyunAudioPlayer] 打开当前播放文件，保持播放状态")
            if playerService.audioPreparationState == .audioReady {
                print("☁️ [AliyunAudioPlayer] 音频已准备完成，无需重新处理")
                return
            }
        }
    ...
    private func prepareAudioPlayback() async {
        ...
        do {
            ...
            await MainActor.run {
                ...
                print("☁️ [AliyunAudioPlayer] ✅ 音频准备完成")
            }
        } catch {
            await MainActor.run {
                isPreparingAudio = false
                ...
                showingErrorAlert = true
                print("☁️ [AliyunAudioPlayer] ❌ 音频加载失败: \(error)")
            }
        }
    }
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    print("☁️ [AliyunAudioPlayer] 找到 \(subtitles.count) 个字幕文件")
                }
            } catch {
                await MainActor.run {
                    isLoadingSubtitles = false
                    print("☁️ [AliyunAudioPlayer] 加载字幕列表失败: \(error)")
                }
            }
        }
    }
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    print("☁️ [AliyunAudioPlayer] ✅ 字幕加载成功: \(subtitles.count) 条")
                }
            } catch {
                await MainActor.run {
                    errorMessage = "字幕加载失败: \(error.localizedDescription)"
                    showingErrorAlert = true
                    print("☁️ [AliyunAudioPlayer] ❌ 字幕加载失败: \(error)")
                }
            }
        }
    }
    ...
    private func onDisappear() {
        print("☁️ [AliyunAudioPlayer] 页面消失，音频继续播放")
    }
    ...
    private func translateSubtitles() async {
        ...
        guard !service.currentSubtitles.isEmpty else { return }
        ...
        await withTaskGroup(of: (Int, String?).self) { group in
            for (index, subtitle) in service.currentSubtitles.enumerated() {
            ...
            var updatedSubtitles = service.currentSubtitles
            ...
            await MainActor.run {
                service.updateSubtitles(updatedSubtitles)
            }
        }
        
        await MainActor.run {
            isTranslating = false
            print("☁️ [AliyunAudioPlayer] ✅ 字幕翻译完成")
        }
    }
...
#Preview {
    NavigationView {
        AliyunAudioPlayerView(file: AliyunMediaFile.example)
    }
}
